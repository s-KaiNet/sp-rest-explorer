...
```bash
slug=$(node ./.opencode/get-shit-done/bin/gsd-tools.js generate-slug "$title" --raw)
```
...
```bash
node ./.opencode/get-shit-done/bin/gsd-tools.js commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
